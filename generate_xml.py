...
def generate_xml_str_head(image_full_name, generated_xml_dir):
    imge_full_name_in_xml = image_full_name.replace('&','$')
    (h,w,c) = get_image_shape(image_full_name)
    xml_str = "<annotation>\n\t\
    <folder>JPEGImages</folder>\n\t\
    <filename>" + imge_full_name_in_xml.split('/')[-1] + "</filename>\n\t\
    " + "<path>" + imge_full_name_in_xml + "</path>\n\t\
    <source>\n\t\t\
...
def generate_xml(std_objs_per_frame, image_full_name, generated_xml_dir, target_classes):
    ...
    for obj in std_objs_per_frame:
        ...
        if((cls_name in target_classes) and confidence > 0.1):
            FOUND_PLATE = True
