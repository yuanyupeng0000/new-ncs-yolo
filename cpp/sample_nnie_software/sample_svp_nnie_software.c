...
static HI_S32 SVP_NNIE_Yolov3_GetResult(SVP_DST_BLOB_S *pstDstBlob,HI_U32 u32GridNumWidth[],
    HI_U32 u32GridNumHeight[],HI_U32 u32EachGridBbox,HI_U32 u32ClassNum,HI_U32 u32SrcWidth,
    HI_U32 u32SrcHeight,HI_U32 u32MaxRoiNum,HI_U32 u32NmsThresh,HI_U32 u32ConfThresh,
    HI_FLOAT af32Bias[3][6],HI_U32* pu32TmpBuf,HI_S32 *ps32DstScores, HI_S32 *ps32DstRoi,
    HI_S32 *ps32ClassRoiNum)
{
    HI_U32 u32BoxsNum = 0;
    HI_FLOAT *pf32BoxTmp = NULL;
    ...
    for (HI_U32 t=0; t<3; t++){
       u32ThisLayerGridNum  = u32GridNumWidth[t]*u32GridNumHeight[t];
       u32HStep = u32GridNumWidth[t];
       ...
       for(i = 0;i < u32ThisLayerBboxNum*u32ParaNum;i++)
       {
           f32InputData[i] = (HI_FLOAT)(ps32InputData[i])/SAMPLE_SVP_NNIE_QUANT_BASE;
       }
       ...
       for(n = 0;n < u32ThisLayerGridNum ;n++)
       {
           ...
           {
               u32Index = (n * u32EachGridBbox + k) * u32ParaNum;
               x = (HI_FLOAT)((w + SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[u32Index + 0])) / u32GridNumWidth[t]); // x
               y = (HI_FLOAT)((h + SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[u32Index + 1])) / u32GridNumHeight[t]); // y
               f32Width = (HI_FLOAT)((exp(pf32BoxTmp[u32Index + 2]) * af32Bias[t][2 * k]) / 416); // w
               f32Height =(HI_FLOAT)((exp(pf32BoxTmp[u32Index + 3]) * af32Bias[t][2 * k + 1]) / 416); // h

               f32ObjScore = SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[u32Index + 4]);
               ///SVP_NNIE_SoftMax(&pf32BoxTmp[u32Index + 5],u32ClassNum);
               for(HI_U32 j=0; j<u32ClassNum; j++){
                   pf32BoxTmp[u32Index + 5 + j] = SAMPLE_SVP_NNIE_SIGMOID(pf32BoxTmp[u32Index + 5 + j]);
               }
...
HI_S32 SAMPLE_SVP_NNIE_Yolov3_Tiny_GetResult(SAMPLE_SVP_NNIE_PARAM_S*pstNnieParam,
    SAMPLE_SVP_NNIE_TINY_YOLOV3_SOFTWARE_PARAM_S* pstSoftwareParam)
{
    return SVP_NNIE_Yolov3_Tiny_GetResult(
        ...
        pstSoftwareParam->u32OriImHeight,
        pstSoftwareParam->u32MaxRoiNum,pstSoftwareParam->u32NmsThresh,
        pstSoftwareParam->u32ConfThresh,pstSoftwareParam->af32Bias2,
        SAMPLE_SVP_NNIE_CONVERT_64BIT_ADDR(HI_U32,pstSoftwareParam->stGetResultTmpBuf.u64VirAddr),
        SAMPLE_SVP_NNIE_CONVERT_64BIT_ADDR(HI_S32,pstSoftwareParam->stDstScore.u64VirAddr),
        SAMPLE_SVP_NNIE_CONVERT_64BIT_ADDR(HI_S32,pstSoftwareParam->stDstRoi.u64VirAddr),
        SAMPLE_SVP_NNIE_CONVERT_64BIT_ADDR(HI_S32,pstSoftwareParam->stClassRoiNum.u64VirAddr));
